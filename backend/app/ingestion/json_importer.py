...
class JSONImporter:
    ...
    def _load_items(self, file_path: str) -> list[Dict[str, Any]]:
    ...
    def import_products_file(self, file_path: str, platform_code: str = "tiki", progress_callback: Optional[Callable[[int, int], None]] = None) -> Dict[str, int]:
        ...
        data = self._load_items(file_path)
        stats["total_rows"] = len(data)
        for i in range(0, len(data), self.batch_size):
            batch_data = []
            for row in data[i:i + self.batch_size]:
                try:
                    norm = DataNormalizer.normalize_product(row, platform_id=platform.id)
            ...
            if progress_callback:
                progress_callback(min(i + self.batch_size, stats["total_rows"]), stats["total_rows"])
        return stats
    ...
    def import_reviews_file(self, file_path: str, platform_code: str = "tiki", progress_callback: Optional[Callable[[int, int], None]] = None) -> Dict[str, int]:
        ...
        for i in range(0, len(data), self.batch_size):
            ...
            for row in data[i:i + self.batch_size]:
                try:
                    ext_prod_id = str(row.get("external_product_id") or row.get("product_id") or "")
                    prod = self.product_repo.get_by_external_id(platform.id, ext_prod_id)
                    if not prod:
                        stats["failed"] += 1
                        continue
                    norm = DataNormalizer.normalize_review(row, product_id=prod.id)
                    norm["platform_id"] = platform.id
                    norm["platform_code"] = platform.code
                    norm["source"] = norm.get("source") or platform.code.upper()
                    norm["external_product_id"] = ext_prod_id
                    norm["product_title"] = getattr(prod, "title", "")
                    if norm["review_text"] and norm["external_review_id"]:
                        batch_reviews.append(norm)
                    else:
                        stats["failed"] += 1
                except Exception:
                    stats["failed"] += 1
